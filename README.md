...
| **pcm_get_team_objectives**    | List the objectives the sponsors set for your team. Takes no team ID: `DYN_objectif` only ever holds the human player's own team objectives, so the tool requires a played career (an unplayed official release errors, having no active player). Per race objective: target type (`WIN`, `TOP_3`, `MAILLOT_SPRINTER`, `STAGE_WIN_1`, `PARTICIPATION`, `VISIBILITY_*`, …), the race it applies to and its dates, the difficulty, and what has been achieved so far — plus the rider who produced that result (objectives are set for the team, not for a rider). Each carries a `status`: `ACHIEVED`, `PARTIAL` (something was achieved but short of the target — a top 5 when the win was asked), `FAILED`, or `IN_PROGRESS` while the race has yet to finish. Ordered by date, earliest race first. Recent editions also set season-long objectives, returned in `seasonObjectives`: the number of wins to collect (`WINS`) and the ranking position to reach (`RANKING`), each with the team's current standing. |
...
2. `pcm_search_cyclist`, `pcm_get_team_roster`, `pcm_get_team_objectives`, `pcm_query_database`, … to explore it.
