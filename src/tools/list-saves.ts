	...
	saves: z
		.array(
			z.object({
				...
				name: z.string().describe("File name, e.g. `MySave.cdb`"),
				lastModified: z
					.string()
					.describe(
		...
		.describe("Discovered saves, newest first"),
});
...
export function registerListSaves(server: McpServer): void {
	server.registerTool(
		"pcm_list_saves",
		{
			title: "List the player's PCM saves",
			description:
				"Discover the player's own Pro Cycling Manager saves on this machine by scanning the `Pro Cycling Manager <year>/Cloud` folders under %APPDATA%, across every installed edition (Windows only). Returns each save's absolute path, file name, last modified date and size (newest first). A save is stored as a `.cdb` database, so its path is what the other tools take as `databasePath` — use `pcm_validate_database` instead to point at any other `.cdb` (official release, community update).",
			outputSchema,
			annotations: {
				readOnlyHint: true,
